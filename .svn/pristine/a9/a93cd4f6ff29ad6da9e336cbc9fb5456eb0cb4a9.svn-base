...
- (id)initWithStationId:(NSDictionary *)stationDic{
    if (self = [super init]) {
        ...
        self.title = [NSString stringWithFormat:@"%@",String(@"故障任务详情")];
...
- (void)getData:(id)obj{
    ...
    parmDic[@"faultTaskId"] = alarmId;
    [requestHelper startRequest:parmDic uri:@"mobile/fault_task/detail" result:^(BResponseModel * _Nonnull respModel) {
...
- (void)buttonClick:(UIButton*)btn{
    ...
    switch (tag) {
        case 5:
            {
                ...
                UIAlertAction * okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    ...
                    [requestHelper stop];
                    NSMutableDictionary *parmDic = [[NSMutableDictionary alloc] init];
                    [parmDic setObject:misstionDic[@"id"] forKey:@"faultTaskId"];
        ...
        {
            NSLog(@"任务处理,%d",tag);
            GZChuliViewController*vc = [[GZChuliViewController alloc] initWithMission:misstionDic];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    switch (indexPath.section) {
        ...
        case 1:
        {
            ...
            if ([misstionDic[@"completeResultNum"] intValue] == 0) {
                if( misstionDic[@"assigneeUserName"] ==nil  ||  [misstionDic[@"assigneeUserName"]   isEqualToString:@""]){
                    return 30;
                }else{
    ...
    switch (section) {
        ...
        case 1:
        {
            if ([misstionDic[@"completeResultNum"] intValue] == 0) {
                ...
                return 5;
            }else if ([misstionDic[@"completeResultNum"] intValue]  == 2) {
                return 7;
            }
// 未处理3 处理中5 处理完成7
            return 3;
        ...
        case 0:
            {
                ...
                switch (indexPath.row) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        dc = @{@"name":@"设备名称：",@"value":TOSTRING(misstionDic[@"deviceInfoNames"])};
                        break;
                    case 3:
                        dc = @{@"name":@"发现人：",@"value":TOSTRING(misstionDic[@"discoverUserName"])};
                        break;
                    case 4:
                        dc = @{@"name":@"发现时间：",@"value":TOSTRING(misstionDic[@"discoverTime"])};
                        break;
                    case 5:

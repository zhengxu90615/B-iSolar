...
@implementation MissionDealContentCell
...
- (void)setData:(NSDictionary *)dic  andNeedLine:(BOOL)need andBlock:(nonnull NormalBlock)blcok
{
    ...
    takeTimeLabel.text = [NSString stringWithFormat:@"接单时间:%@",dic[@"takingTime"]];
    paidanLabel.text = [NSString stringWithFormat:@"派单时间:%@",dic[@"dispatchTime"]];
    ...
    for (int i=0; i<lines; i++) {
        for (int j =0; j<3; j++) {
            ...
            if (index < fileListCount) {
                UIImageView *imv = [[UIImageView alloc] initWithFrame:CGRectMake(100 + (picwidth+20)*j, labelsize.height + 89 + (picH+10)*i  , picwidth, picH)];

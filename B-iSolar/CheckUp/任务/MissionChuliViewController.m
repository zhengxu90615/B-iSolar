...
@interface MissionChuliViewController ()<CustomDatePickerDelegate>
{
...
@implementation MissionChuliViewController
...
- (id)initWithMission:(NSDictionary*)miss
{
    if (self = [super init]) {
        ...
        misstionDic[@"rate"] = misRate;
        misstionDic[@"rate1"] = misRate;
...
- (void)getData:(id)sender{
    ...
    [requestHelper startRequest:parmDic uri:@"mobile/mission_trace/detail" result:^(BResponseModel * _Nonnull respModel) {
        if (respModel.success) {
            ...
            misRate = TOSTRING(respModel.data[@"processRate"]);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section ==0) {
        ...
        _atest.text = misstionDic[@"mission_trace_detail"][@"missionNote"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        [cell setName2:misstionDic[@"mission_trace_detail"][@"name"] detail:misstionDic[@"mission_trace_detail"][@"missionNote"]];
        return cell;
    }else{
            ...
            [cell setDataAndName:dic andCanEdit:YES andBlock:^(id x) {
                if (  [x intValue]  >= [misstionDic[@"rate1"] intValue] &&  [x intValue]  <= 100  ) {
                    ...
                    if ([misRate isEqualToString:@"100"]) {
                        ...
                        UIAlertController *alertController =  [UIAlertController alertControllerWithTitle:String(@"提示") message:String(@"确认是否已完成该任务") preferredStyle:UIAlertControllerStyleAlert] ;
...
//    ["missionTraceId", "dealTime", "dealNote", "rate", "fileList"])
    parmDic[@"dealTime"] = misDT;
    parmDic[@"rate"] = misRate;
    parmDic[@"dealNote"] = misNote;
    parmDic[@"fileList"] = @"[]";
        ...
        dispatch_async(serialQueue, ^{
            ...
            for (id object in picArr) {
                if ([object isKindOfClass: [UIImage class]]) {
                    ...
                    /////// uiload 。。。。。。
                    [helper startUploadImage2:object  withP: @{} uri:@"mobile/mission_trace/file_upload" result:^(BResponseModel * _Nonnull respModel) {
            ...
            [requestHelper startRequest:parmDic uri:@"mobile/mission_trace/deal" result:^(BResponseModel * _Nonnull respModel) {
                if (respModel.success) {

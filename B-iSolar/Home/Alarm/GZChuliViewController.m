...
@interface GZChuliViewController ()<CustomDatePickerDelegate>
{
...
@implementation GZChuliViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            dc = @{@"name":@"故障描述：",@"value":TOSTRING(misstionDic[@"description"])};
        }
...
- (void)buttonClick:(id)sender{
    ...
    [requestHelper stop];
    NSMutableDictionary *parmDic = [[NSMutableDictionary alloc] init];
    [parmDic setObject:missId forKey:@"faultTaskId"];
                ...
                dispatch_async(serialQueue, ^{
                    ...
                    for (id object in picArr) {
                        if ([object isKindOfClass: [UIImage class]]) {
                            ...
                            /////// uiload 。。。。。。
                            [helper startUploadImage2:object  withP: @{} uri:@"mobile/fault_task/single_file_upload" result:^(BResponseModel * _Nonnull respModel) {
                    ...
                    [requestHelper startRequest:parmDic uri:@"mobile/fault_task/deal" result:^(BResponseModel * _Nonnull respModel) {
                        if (respModel.success) {

...
@interface HomeViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,ZBarReaderDelegate>
{
    CGRect bounds;
    int alarmNum;
    int noticeNum;
...
static NSString *homeTableViewElecCellInd = @"HomeTableViewElecCell";
...
- (void)viewDidLoad {
    ...
    self.title = String(@"首页");
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(userLoginNoti:) name:NOTIFICATION_LOGIN object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getHeader) name:@"push" object:nil];

    alarmNum = noticeNum = 0;
    ...
    [self.mainTableV.mj_header beginRefreshing];
     ...
     self.navigationItem.leftBarButtonItem=[[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"bisolar_announcement"] imageChangeColor:MAIN_TINIT_COLOR WithNumberRight:@""] style:UIBarButtonItemStylePlain target:self action:@selector(showMess)];
    
    self.view.backgroundColor = MAIN_BACKGROUND_COLOR;
    mainTableV.backgroundColor = [UIColor clearColor];
    
    [mainTableV registerNib:[UINib nibWithNibName:@"HomeTableViewMyStationCell" bundle:nil] forCellReuseIdentifier:homeTableViewMyStationCellInd];
    [mainTableV registerNib:[UINib nibWithNibName:@"HomeTableViewElecCell" bundle:nil] forCellReuseIdentifier:homeTableViewElecCellInd];
...
- (void)setHeader{
    ...
    if (1) {
        ...
        [myBtn setImage:[UIImage imageNamed:@"icon_nav_ saoyisao"] forState:UIControlStateNormal];
...
- (void)setNum{
    ...
    [[headView viewWithTag:1] removeFromSuperview];
    [[headView viewWithTag:2] removeFromSuperview];
...
- (void)getData:(id)obj{
    ...
    [apiBlock startRequest:nil uri:API_HOME result:^(BResponseModel * _Nonnull respModel) {
        if (respModel.success) {
            ws.dataModel = respModel;
            ws.navigationItem.leftBarButtonItem=[[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"bisolar_announcement"] imageChangeColor:MAIN_TINIT_COLOR WithNumberRight:TOSTRING(respModel.data[@"announcementsCount"])] style:UIBarButtonItemStylePlain target:self action:@selector(showMess)];
        }else{
...
//    self.navigationItem.rightBarButtonItems = @[[[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"icon-scan"] imageChangeColor:MAIN_TINIT_COLOR] style:UIBarButtonItemStylePlain target:self action:@selector(scan)],[[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"icon_mine"] imageChangeColor:MAIN_TINIT_COLOR] style:UIBarButtonItemStylePlain target:self action:@selector(showMy)]];
    
}

- (void)getHeader{
    weak_self(ws)
    BHttpRequest * apiBlock2 = [BHttpRequest new];
      apiBlock2.needShowHud =@0;
      [apiBlock2 startRequest:nil uri:API_NOTICE_NUM result:^(BResponseModel * _Nonnull respModel) {
         if (respModel.success) {
             noticeNum = [respModel.data[@"all_num"] intValue];
             [ws setNum];
         }
      }];
      
    BHttpRequest * apiBlock3 = [BHttpRequest new];
     apiBlock3.needShowHud =@0;
     [apiBlock3 startRequest:nil uri:API_ALARMS_NUM result:^(BResponseModel * _Nonnull respModel) {
        if (respModel.success) {
            alarmNum = [respModel.data[@"all_num"] intValue];
            [ws setNum];
        }
     }];
}
...
//    return 10;
    ...
    switch (indexPath.section) {
        ...
        case 2:
        {
            CGFloat img_w = (MAINSCREENWIDTH-60)/3;
            CGFloat img_h = img_w *73 / 105;
            return img_h + 14 + 47.5;
        }
            break;
        case 3:
            return 361;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.section) {
        case 0:
        {
            HomeTableViewMyStationCell *cell = [tableView dequeueReusableCellWithIdentifier:homeTableViewMyStationCellInd];
        ...
        case 2:
        {
            HomeTableViewElecCell *cell = [tableView dequeueReusableCellWithIdentifier:homeTableViewElecCellInd];
            weak_self(ws)
            [cell setData:dataModel andBlock:^(id x) {
                switch ([x intValue]) {
                    case 0:
                        ...
                        case 2:
                        {
                            HeroTopViewController *vc =[[HeroTopViewController alloc]init] ;
                            PUSHNAVICONTROLLER(vc)
                        }
                        break;
        ...
        case 3:
        {
            HomeComTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:homeTableViewCompleteCellInd];
        ...
        case 1:
        {
            HomeTableChartsCell *cell = [tableView dequeueReusableCellWithIdentifier:homeTableChartsCellInd];
            weak_self(ws)
            [cell setData:dataModel];
            ...
            cell.clickBlock = ^(NSInteger type, NSString * _Nonnull data) {
                if (type == 0) {
                    ws.tabBarController.selectedIndex = 1;
                }else{
                    ws.tabBarController.selectedIndex = 2;
                    ReportViewController *reportV = ((UINavigationController *)ws.tabBarController.viewControllers[2]).viewControllers[0];
                    [reportV loadDataOfDay:data];
                }
            };
            return cell;
        }

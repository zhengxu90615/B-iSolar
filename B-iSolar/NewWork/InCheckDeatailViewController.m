...
#import "MarkAlert.h"
@interface InCheckDeatailViewController ()<UITextFieldDelegate>
{
...
@implementation InCheckDeatailViewController
...
//    ["ExternalStation_id", "ExternalStation_type", "dataList", "inspectionPer", "inspectionDate"]
    ...
    parmDic[@"type"] = currentType[@"id"];
   
    
    parmDic[@"inspectionPer"] = peopleLabel.text;
    parmDic[@"inspectionPerIds"] = [selectedUserList componentsJoinedByString:@","];
    ...
    weak_self(ws);
    [requestHelper startRequest:parmDic uri:@"mobile/inspectionReportSaveApi" result:^(BResponseModel * _Nonnull respModel) {
...
- (void)headCheckClick:(int)index
{
    ...
    if (index == 4) {
        if (userSelectList) {
            ...
            NSString *ids = [selectedUserList componentsJoinedByString:@","];

            NSMutableArray *indexArr = [[NSMutableArray alloc] init];
            
            for (int i = 0; i<userSelectList.count; i++) {
                if ([ids containsString:[NSString stringWithFormat:@"#%@#",userSelectList[i][@"id"]] ]) {
                    [indexArr addObject:@(i)];
                }
            }
            
            
            MarkAlert *al = [[MarkAlert alloc] initWithPayAction:@"请选择检查人员" action:userSelectList result:^(NSArray *arr) {
                if (arr) {
                    NSLog(@"%@",arr);
                    selectedUserList = [NSMutableArray array];
                    NSMutableArray *nameArr = [NSMutableArray array];
                    for (NSDictionary *dic in arr) {
                        [selectedUserList addObject:[NSString stringWithFormat:@"#%@#",dic[@"id"]]];
                    ...
                    peopleLabel.text = [nameArr componentsJoinedByString:@","];
                }
        ...
        [requestHelper startRequest:parmDic uri:@"mobile/orga_sta_list" result:^(BResponseModel * _Nonnull respModel) {
            if (respModel.success) {
                ...
                for (NSDictionary * dic in respModel.data) {
                    ...
                    [areaArr addObject:newDic];
                }
                [ws.pickerV setType:PickerTypeTwo andTag:0 andDatas:areaArr];
                self.pickerV.hiddenCustomPicker = NO;
            }else{
                [SVProgressHUD showErrorWithStatus:respModel.errorMessage?respModel.errorMessage:respModel.message];
            }
...
- (void)getStationUser:(NormalBlock) b{
    
    if (currentStation) {
        BHttpRequest *req = [BHttpRequest new];
        req.needShowHud =@0;
        [req startRequest:@{@"station_id":currentStation[@"id"]} uri:@"mobile/inspectionReportUserSelectApi" result:^(BResponseModel * _Nonnull respModel) {
            if (respModel.success) {
                userSelectList = nil;
                userSelectList = [[NSMutableArray alloc] init];

                for (NSDictionary *dic in respModel.data[@"userList"]) {
                    [userSelectList addObject:@{@"id":dic[@"user_id"],@"name":dic[@"user_name"]}];
                }
                
//                (1, "集中式"),(2, "分布式")
                
                
                currentType = @{@"id":respModel.data[@"ExternalStation_type"] , @"name": [respModel.data[@"ExternalStation_type"] intValue]==1?@"集中式":@"分布式"    };
                
                nameLabel.text = currentType[@"name"];
...
- (void)getData:(id)sender{
    ...
    [requestHelper startRequest:parmDic uri:@"mobile/inspectionReportDetailApi" result:^(BResponseModel * _Nonnull respModel) {
        if (respModel.success) {
...
//            peopleLabel.text =ws.dataModel.data[@"inspectionPer"];
            ...
            nameLabel.text = ws.dataModel.data[@"ExternalStation_type_name"];
            peopleLabel.text =ws.dataModel.data[@"inspectionPer"];
            
            if(dataDic)
            {
                NSDictionary *dic =ws.dataModel.data;
                [checkHeadView setTitle0:dic[@"ExternalStation_area_name"] title1:ws.dataModel.data[@"ExternalStation_name"] title2:ws.dataModel.data[@"inspectionDate"]];
                
                currentArea = @{@"id":dic[@"ExternalStation_area_id"] , @"name":dic[@"ExternalStation_area_name"]};
                currentStation = @{@"id":dic[@"ExternalStation_id"] , @"name":dic[@"ExternalStation_name"]};
                currentType = @{@"id":dic[@"ExternalStation_type"] , @"name":dic[@"ExternalStation_type_name"]};

                currentDate = dic[@"inspectionDate"];
                selectedUserList = [[NSMutableArray alloc] initWithArray:[dic[@"inspectionPerIds"] componentsSeparatedByString:@","]];
                [self getStationUser:nil];
...
- (void)  imagePickerController: (UIImagePickerController*) picker didFinishPickingMediaWithInfo: (NSDictionary*) info
{
        ...
        [requestHelper startUploadImage:image withP:parmDic uri:@"mobile/inspectionReportPicUploadApi" result:^(BResponseModel * _Nonnull resp) {
             if (resp.success) {
...
//       [self.mainTableV.mj_header beginRefreshing];
   }else if (v.tag==0)
   {
       ...
       if([newArea[@"id"] intValue] !=  [currentArea[@"id"] intValue])
       {
           ...
           nameLabel.text = @"";  // 电站类型
           peopleLabel.text = @""; // 检查人员
       }else{
           
       }
      
       [checkHeadView setTitle0:currentArea[@"name"] title1:currentStation[@"name"] title2:currentDate];
       [self getStationUser:nil];
//       [self.mainTableV.mj_header beginRefreshing];

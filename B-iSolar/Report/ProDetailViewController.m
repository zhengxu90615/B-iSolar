...
- (id)initWithDate:(NSString *)selectDateString
{
    if (self = [super init]) {
...
- (void)getData:(id)obj{
    ...
    [apiBlock startRequest:parmDic uri:API_REPORT_PROD_COMPLETE result:^(BResponseModel * _Nonnull respModel) {
...
- (IBAction)sepBtnClick:(UIButton *)sender {
    ...
    if (sender == monthBtn) {
        if (arr.count == 1) {
            ...
            [arr addObject:[lastMonth componentsSeparatedByString:@"-"][1]];
        }
...
- (void)selectDate{
    if (currentSelect == 1) {
        [self.pickerV setType:PickerTypeYYYYMM andTag:1 andDatas:nil];
    }else{
        [self.pickerV setType:PickerTypeYYYY andTag:2 andDatas:nil];
    }
    NSMutableArray *arr = [[NSMutableArray alloc] initWithArray:[selectTime componentsSeparatedByString:@"-"]];
...
{
    if (dataModel.data) {
        return [dataModel.data[@"list"] count];
    }
    return 0;
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    NSArray *orArr = dataModel.data[@"list"];
    return [orArr[section][@"list"] count] +2;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    NSString *title = dataModel.data[@"list"][section][@"name"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell setData:dataModel andIndex:indexPath];
    return cell;

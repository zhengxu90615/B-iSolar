...
- (void)gradient {
    ...
    leftGradient.colors = @[(id)MAIN_TINIT_COLOR.CGColor,
                            (id)MAIN_TINIT_COLOR.CGColor];
    ...
    rightGradient.colors = @[(id)MAIN_TINIT_COLOR.CGColor,
                             (id)MAIN_TINIT_COLOR.CGColor];
...
- (UILabel *)progressLabel {
    if (!_progressLabel) {
        ...
        _progressLabel.font = FONTSIZE_TABLEVIEW_CELL_DESCRIPTION2;
...
- (void)setProgress:(CGFloat)progress andTitle:(NSString *)title{
    _progress = progress;
    _progressLabel.text = [NSString stringWithFormat:@"%.2f%%\n%@",progress * 100,title];
